...
ASP.NET 5는 여러 프레임워크를 대상으로 개발이 가능하기 때문에 애플리케이션을 다른 종류의 호스팅 환경에 배포할 수 있다. 기본적으로 애플리케이션은 .NET 풀 버전을 대상으로 하지만 닷넷 코어로 대상을 변경할 수 있다. 대부분의 레거시 앱들은 닷넷 코어에서 지원하지 않는 BCL에 의존하고 있을 가능성이 크기 때문에 적어도 초기에는 풀 버전 ASP.NET 5를 대상으로 할 것이다.  닷넷 코어는 웹 앱에 최적화된 경량 버전의 프레임웤이고 리눅스와 맥 환경을 지원한다. 닷넷 코어는 애플리케이션과 함께 배포될 수 있어 다른 버전의 닷넷 코어를 대상으로 하는 앱들이 동일 서버에서 동작할 수 있다. 이 것은 또한 별도 NuGet 패키지인 모듈로써 필요에 의해 기능을 추가할 수 있다 (:doc:`learn more about .NET Core <dotnetcore>`). 
